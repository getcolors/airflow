(ns pin
  ...
  This launcher pins only airflow. `once` and `green` come transitively from
  airflow's deps.edn, so there is one site here rather than ONCE's three."
  (:require
...
(def ^:private site
  "Where a copied payload records which airflow commit to resolve.
...
(defn- repo-head
  ...
  something nobody else can fetch, which fails at the worst possible moment — in
  someone else's project, on first run."
  [dir label]
  ...
  (let [file (io/file (:path site))]
    (cond
      ...
      (let [text (slurp file)
        ...
        (if-not current
          {:green/exit 2
           :green/err (str "could not locate airflow-sha in " (:path site))}
          (let [[head err] (repo-head "." "airflow")]
            (cond
