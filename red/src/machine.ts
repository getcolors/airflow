/** Airflow's single host requirement and application parameter adapter. */
...
export function requirements(opts: Opts) {
  ...
  return {single_host:true,private:false,security:{ingress,egress:'all',private_filter:false},legacy_state_keys:[String(opts.profile)+'/airflow-compute.tfstate']};
}
...
export async function step(opts: Opts): Promise<Opts> {
  ...
  if(planning)for(const [stage,documents] of [['shared',result.documents.shared],...Object.entries(result.documents.nodes).map(([id,docs])=>['nodes/'+id,docs])] as [string,Record<string,unknown>][]){
    ...
    const target=join(stageDir(opts,'airflow-compute'),stage,filename);mkdirSync(dirname(target),{recursive:true});writeFileSync(target,JSON.stringify(sorted(document),null,2)+'\n');
  }}
