...
slots=["provider-smtp","provider-dns"]
...
def state_errors(o):
 e=machine.errors(o)
 if o.get("digitalocean-firewall") is False:e.append("digitalocean-firewall=false is retired; compute firewall rules are required")
 ...
 return e
def secret_errors(o):return [f"required credential is not set: {par_name(k)}" for k in dict.fromkeys([*own_secrets,*keys(o,'secrets'),*[v.removeprefix('COLORS_PAR_').lower().replace('_','-') for v in credential_requirements(o)]]) if placeholder(o.get(k))]
