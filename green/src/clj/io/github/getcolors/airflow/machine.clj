(ns io.github.getcolors.airflow.machine
...
(defn requirements [opts]
  {:single_host true :private false
   ...
   :legacy_state_keys [(str (:profile opts) "/airflow-compute.tfstate")]})
(defn errors [opts]
  ...
  (let [planning? (or (= :build (:green/event opts)) (:green/dry-run opts))
    ...
    (when planning?
      (doseq [[stage documents] (cons ["shared" (get-in result [:documents :shared])]
        ...
        (let [target (io/file (cli/stage-dir opts "airflow-compute") stage filename)]
